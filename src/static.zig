...
fn middlewareHandler(ptr: ?*anyopaque, req: *Request, res: *Response, next: *ziez.Next) void {
    const config: *StaticConfig = @ptrCast(@alignCast(ptr.?));
    const served = handle(req, res, config.*) catch {
        next.call();
        return;
    };
    if (!served) next.call();
...
fn middlewareDeinit(ptr: ?*anyopaque, _: std.mem.Allocator) void {
    if (ptr) |p| std.heap.page_allocator.destroy(@as(*StaticConfig, @ptrCast(@alignCast(p))));
}

pub fn asMiddleware(config: StaticConfig) ziez.Middleware {
    const cfg = std.heap.page_allocator.create(StaticConfig) catch @panic("ziez-static: OOM");
    cfg.* = config;
    return .{
        .ptr = cfg,
        .handler = middlewareHandler,
        .deinit_fn = middlewareDeinit,
    };
}
